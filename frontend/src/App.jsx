...
const UserForm = ({ user, companies, onSubmit }) => {
  ...
  return (
    <form onSubmit={(e) => {
      ...
      {user?.role === 'SYSTEM_ADMIN' && (
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <select ref={roleRef} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-4 rounded-xl outline-none text-slate-900 dark:text-white">
          ...
          <div className="space-y-1.5">
            ...
            <select ref={companyIdRef} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-4 rounded-xl outline-none text-slate-900 dark:text-white">
      ...
      <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-bold transition-all mt-4">Erişim Yetkisi Ver</button>
    </form>
  );
};
...
function App() {
      ...
      html: `
        <div style="text-align:left; font-family:monospace; font-size:11px; color:${isDark ? '#94a3b8' : '#64748b'}; line-height:1.6; margin-bottom:12px">
          ...
          <div style="margin-top:8px; opacity:0.8">
            ...
            • <span style="color:#f97316">alan:değer</span> tek alanı kilitle
          </div>
        </div>
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white flex transition-colors duration-500">
      {/* Sidebar */}
      <aside className="w-64 h-screen flex-shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-xl z-20 transition-colors duration-500 overflow-y-auto">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => { setView('dashboard'); setSelectedSensor(null); fetchDashboardData(); }}
          ...
          {(user?.role === 'SYSTEM_ADMIN' || user?.role === 'COMPANY_ADMIN') && (
            <button onClick={() => { setView('admin'); fetchAdminData(); fetchActivityStats(); }}
          ...
          {(user?.role === 'SYSTEM_ADMIN' || user?.role === 'COMPANY_ADMIN') && (
            <button onClick={() => { setView('logs'); fetchLogs(); }}
      ...
      {/* Main Content */}
      <main className="flex-1 p-8 overflow-y-auto min-w-0 min-h-0">
        {view === 'dashboard' && (
          <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              ...
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                <button
                  ...
                  className="flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-5 py-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all font-bold text-slate-700 dark:text-slate-200 min-w-[220px] justify-between shadow-lg dark:shadow-none"
                >
        ...
        {view === 'admin' && (
          <div className="space-y-12 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-black bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-400 bg-clip-text text-transparent italic tracking-tight uppercase">Sistem Yönetimi</h1>
                <p className="text-slate-500 text-xs mt-1 font-bold uppercase tracking-widest opacity-80">Kurumsal kontrol ve yetkilendirme portalı</p>
              </div>
              <div className="flex gap-4">
                {user?.role === 'SYSTEM_ADMIN' && (
                  <button onClick={() => setShowCompanyForm(true)} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:border-blue-500 transition-all shadow-lg dark:shadow-none">
                    + Yeni Kuruluş
                  </button>
                )}
                <button onClick={() => setShowUserForm(true)} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:border-blue-500 transition-all shadow-lg dark:shadow-none">
                  + Yeni Kimlik
                </button>
                <button onClick={() => setShowSensorModal(true)} className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-500 transition-all shadow-lg shadow-blue-600/20">
                  + Düğümü Kaydet
                </button>
              </div>
            </div>
            ...
            <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-xl overflow-hidden relative">
              <div className="absolute top-0 right-0 p-8 opacity-5 font-black text-6xl tracking-tighter uppercase pointer-events-none">TRAFİK_ŞEMASI</div>
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="space-y-4">
              {logs.length > 0 ? logs.map(log => (
                <div key={log.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl flex items-center justify-between hover:border-blue-500 transition-all shadow-sm dark:shadow-none group">
                  <div className="flex items-center gap-6">
                    <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-950 flex items-center justify-center text-blue-600 dark:text-blue-400 font-black shadow-inner border border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex items-center gap-12">
                    ...
                    <div className="text-right flex flex-col items-end gap-1">
                      <div className="text-[10px] font-black text-slate-900 dark:text-slate-300 uppercase tracking-tighter">
...
const Pagination = ({ meta, onPageChange }) => {
  ...
  return (
    <div className="flex items-center justify-center gap-4 mt-6 py-4 border-t border-slate-100 dark:border-slate-800/50">
      <button
